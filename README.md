...
## 4. Код hitbox:
### Основные функции:
#### 1. initialize(canv) - Инициализирует игровую сцену, принимая холст Tkinter в виде аргумента canv. Внутри выполняются следующие шаги:
#### 2. _get_screen_text() - Отвечает за формирование текста статуса игры
#### 3._update_screen_text() - Обновляет текст индикатора на холсте, вызывая _get_screen_text() и применяя полученный текст к соответствующему элементу холста.
#### 4.get_player() - Возвращает первый танк из списка _tanks, предполагаемый игрок.
#### 5.update() - Основная игровая логика цикла обновления:Сначала обновляется текстовое окно с помощью _update_screen_text(),Далее начинается процесс обработки всех танков на карте:,Если танк уничтожен (кроме первого игрока), он удаляется из списка.Оставшиеся танки обновляют свое состояние и проверяют столкновения друг с другом и с выпущенными ракетами.
#### 6.check_collision(tank) - Проверяет, сталкивался ли данный танк с любым другим танком. Если обнаружены пересечения, возвращается True.
#### 7. spawn(is_bot=True) - Спавнит новый танк на карте:Случайно выбирает свободную клетку карты (исключая крайние линии и заблокированные участки).Создается объект класса Tank, который помещается в список _tanks.

## Код написан на python и с библиотекой trkinter 
